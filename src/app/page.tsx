...
export default function HomePage() {
  ...
  const handleDemoState = (step: 'upload' | 'analyze' | 'quiz' | 'video' | 'preview') => {
    // Fix SSR issue - tylko w przeglądarce
    const createDemoFile = (name: string) => {
      if (typeof window !== 'undefined') {
        return new File([], name);
      }
      return null as any; // Fallback dla SSR
    };

    const demoImages: CarImage[] = [
      {
        id: '1',
        file: createDemoFile('bmw.jpg'),
      ...
      {
        id: '3',
        file: createDemoFile('mercedes.jpg'), 
      ...
      {
        id: '4',
        file: createDemoFile('porsche.jpg'),
            ...
            <div className="flex flex-wrap gap-2">
              {APP_STEPS.map((step) => (
                <Button
                  ...
                  size="sm"
                  onClick={() => handleDemoState(step.id as 'upload' | 'analyze' | 'quiz' | 'video' | 'preview')}
                >
                  {step.title}
                </Button>
              ))}
